...
class OTFController
{
    ...
    function drawGps(dc, gpsAccuracy, x, y) {
    	// gps 
    	if (gpsAccuracy==null) {
    		gpsAccuracy = 0;
    	}
        ...
        if (gpsAccuracy < 2) {
            drawGpsSign(dc, x, y, inactiveGpsBackground, inactiveGpsBackground, inactiveGpsBackground);
        } else if (gpsAccuracy == 2) {
            drawGpsSign(dc, x, y, batteryColor1, inactiveGpsBackground, inactiveGpsBackground);
        } else if (gpsAccuracy == 3) {          
            drawGpsSign(dc, x, y, batteryColor1, batteryColor1, inactiveGpsBackground);
        } else {
            drawGpsSign(dc, x, y, batteryColor1, batteryColor1, batteryColor1);
        }
    }
    
    hidden function drawGpsSign(dc, xStart, yStart, color1, color2, color3) {
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.drawRectangle(xStart - 1, yStart + 10, 6, 5);
        dc.setColor(color1, Graphics.COLOR_TRANSPARENT);
        dc.fillRectangle(xStart, yStart + 11, 4,3);
        
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.drawRectangle(xStart + 6, yStart + 7, 6, 8);
        dc.setColor(color2, Graphics.COLOR_TRANSPARENT);
        dc.fillRectangle(xStart + 7, yStart + 8, 4, 6);
        
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.drawRectangle(xStart + 13, yStart + 3, 6, 12);
        dc.setColor(color3, Graphics.COLOR_TRANSPARENT);
        dc.fillRectangle(xStart + 14, yStart + 4, 4, 10);
    }
}